async function battlemanager(move) {
    ...
    p1isfaster = determinemoveorder()
        ...
        whoismoving(p1isfaster ? 'friend' : 'foe')
        battlemessage = uname  + ' brukte ' + (p1isfaster ? p1move.name : p2move.name) + '!'
        updateview()
        await delay(2000)
        await eval(p1isfaster ? p1move.movetype : p2move.movetype + "()")
        await delay(2000)
        ...
        updateview()
        await delay(2000)
        await eval(!p1isfaster ? p1move.movetype : p2move.movetype + "(p2move)")
        await delay(2000)
    endofround()
}
...
async function endofround() {
    ...
    faster = p1isfaster ? p1 : p2
    slower = !p1isfaster ? p1 : p2
...
async function endofrounddamage(what, value) {
    let faster = p1isfaster ? p1.pokemon[0] : p2.pokemon[0]
    let slower = !p1isfaster ? p1.pokemon[0] : p2.pokemon[0]
    let fasterstat = p1isfaster ? player : rival
    let slowerstat = !p1isfaster ? player : rival
...
function deathmanager(value) {
    value = isp1 ? player : rival
    stats = ['atk', 'def', 'satk', 'sdef', 'spe', 'acc', 'eva']
    stats.forEach(stat => {
        target[stat] = 6
    })
    target.toxcounter = 1
    if (value) changepokemon('dead') 
    else {
